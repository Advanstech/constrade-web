...
project-root/
  ...
  ├── supabase/
  │   └── functions/         # Enter Cloud backend functions (markets, trading, account, onboarding, admin)
  │       └── _shared/       # Source shared modules (inlined into each deployed function)
  ├── src/
  │   ├── auth/              # AuthProvider (session + role context), RequireAuth/RequireRole guards
  │   ├── components/        # All reusable UI components
  │   │   ├── brand/         # Constant Capital logo
  │   │   ├── layout/        # SiteHeader (marketing), AppShell (trading), Footer, PageHeader
  │   │   ├── market/        # PriceTicker, MarketTable pieces, Sparkline, StatCard
  │   │   └── ui/            # Prebuilt and custom UI components, grouped by function
  │   ├── hooks/             # Custom React hooks
  │   ├── i18n/              # i18n runtime: config.ts (entry) + util.ts (helpers)
  │   ├── integrations/      # Generated Enter Cloud client (do not edit)
  │   ├── lib/               # api.ts (typed backend-function client), api.types.ts,
  │   │                      # format.ts, permissions.ts, utils.ts
  │   ├── pages/             # Application pages (each page in its own subdirectory)
  │   │   ├── marketing/     # Public landing site
  │   │   ├── auth/          # Login / Register + AuthShell
  │   │   ├── onboarding/    # CSD onboarding wizard
  │   │   ├── app/           # Client portal (dashboard, markets, trade, portfolio, orders, funding, profile)
  │   │   └── admin/         # Staff portal (dashboard, users, orders, reports)
